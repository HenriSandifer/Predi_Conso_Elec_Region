...
def render_prediction_tab():
    ...
    st.title(f"🔮 Prevision de Consommation Electrique du {fr_date_dmy}")
    ...
    region = st.selectbox("Select Region", sorted(region_mapping.keys()), index=0)
    ...
    run_time_hr = st.selectbox("Select Prediction Run Time", available_run_times, index=available_run_times.index(default_run_time))
    ...
    st.caption(f"Loading plot from: '{plot_key}'")

    data = read_json_from_s3(plot_key)

    if not data:
        st.error("⚠️ Could not load prediction file.")
        return
