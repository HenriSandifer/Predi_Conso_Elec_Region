...
def plot_pred(region_abbr_caps, region_abbr_lwrc, target_month, chosen_day, run_time_hr):
    ...
    prediction_files = list(pred_dir.glob("*full*.csv"))

    if not prediction_files:
        print(f"⚠️ No full-day prediction found for run time {run_time_hr}.")
        return

    csv_path = prediction_files[0]
    ...
    plot_path = pred_dir / f"plot_pred_full_{region_abbr_lwrc}_{date_ymd}_{run_time_hr}.json"
    with open(plot_path, "w", encoding="utf-8") as f:
        f.write(plot_json)

    print(f"✅ Plot saved locally at: {plot_path}")
