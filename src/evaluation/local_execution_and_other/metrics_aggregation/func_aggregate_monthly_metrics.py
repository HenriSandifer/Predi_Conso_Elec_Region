...
def aggregate_monthly_metrics(region_abbr_caps, target_month, region_abbr_lwrc, bucket_name="predi-conso-elec-region"):
    """
    Aggregate all evaluation metric CSVs for a region and month into a single CSV file on S3.
    """
    s3 = boto3.client("s3")
    prefix = f"Predictions/{region_abbr_caps}/{target_month}/"
    ...
    output_key = f"Predictions/{region_abbr_caps}/{target_month}/metrics_master_{target_month}_{region_abbr_lwrc}.csv"
